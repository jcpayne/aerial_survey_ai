...
def run(request):
    try:
        if request.method == 'POST':
            ...
            pil_image = Image.open(BytesIO(reqBody)) #PIL can read from an IO stream but cv2 can't
            image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)
            result = model(image)
            if (len(result['instances']) > 0):
                visualizer = Visualizer(image[:, :, ::-1], metadata=test_metadata, scale=1.0)
                ...
                #return_object = {'prediction':result,'annotated_img':annotated_img.tolist()}
                return {'annotated_img':annotated_img.tolist()}
            else:
                #return_object = {'prediction':result,'annotated_img':image.tolist()}                
                return {'annotated_img':image.tolist()}
        else:
