...
def init():
    ...
    cfg = setup_model_configuration(modeldir, "dummy_output_dir", CLASS_NAMES)
...
def run(request):
    ...
    try:
        if request.method == 'POST':
            ...
            #TODO: Get the filename as a string, and go find the file
            #Open the image with PIL and copy its array
            image_files = json.loads(reqBody)['file_list']
            imagedir = Path(os.getenv('AZUREML_MODEL_DIR'))/'images_to_process'
            #Build evaluator
            ...
            for f in image_files:
                ...
                datasetdicts = [{"image_id":f, "trow":tile_pos["trow"],"tcol":tile_pos["tcol"],"image":tile,\
                                 "width":tile_size[1],"height":tile_size[0]} for tile_pos,tile in zip(tile_positions,tilelist)]
                ...
                file_results = combine_tile_annotations(tile_results,tile_size,tile_overlap,fullimage_size)
